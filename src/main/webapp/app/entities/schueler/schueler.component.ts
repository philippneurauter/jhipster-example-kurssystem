import { Component, OnInit, OnDestroy } from '@angular/core';
import { HttpErrorResponse, HttpResponse } from '@angular/common/http';
import { Subscription } from 'rxjs';
import { filter, map } from 'rxjs/operators';
import { JhiEventManager, JhiAlertService } from 'ng-jhipster';

import { ISchueler } from 'app/shared/model/schueler.model';
import { AccountService } from 'app/core';
import { SchuelerService } from './schueler.service';

@Component({
    selector: 'jhi-schueler',
    templateUrl: './schueler.component.html'
})
export class SchuelerComponent implements OnInit, OnDestroy {
    schuelers: ISchueler[];
    currentAccount: any;
    eventSubscriber: Subscription;

    constructor(
        protected schuelerService: SchuelerService,
        protected jhiAlertService: JhiAlertService,
        protected eventManager: JhiEventManager,
        protected accountService: AccountService
    ) {}

    loadAll() {
        this.schuelerService
            .query()
            .pipe(
                filter((res: HttpResponse<ISchueler[]>) => res.ok),
                map((res: HttpResponse<ISchueler[]>) => res.body)
            )
            .subscribe(
                (res: ISchueler[]) => {
                    this.schuelers = res;
                },
                (res: HttpErrorResponse) => this.onError(res.message)
            );
    }

    ngOnInit() {
        this.loadAll();
        this.accountService.identity().then(account => {
            this.currentAccount = account;
        });
        this.registerChangeInSchuelers();
    }

    ngOnDestroy() {
        this.eventManager.destroy(this.eventSubscriber);
    }

    trackId(index: number, item: ISchueler) {
        return item.id;
    }

    registerChangeInSchuelers() {
        this.eventSubscriber = this.eventManager.subscribe('schuelerListModification', response => this.loadAll());
    }

    protected onError(errorMessage: string) {
        this.jhiAlertService.error(errorMessage, null, null);
    }
}
